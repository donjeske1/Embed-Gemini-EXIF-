...
const MetadataItem: React.FC<MetadataItemProps> = ({ label, value, isCode = false, isMono = false }) => (
    <div>
        <h4 className="font-semibold text-sm text-slate-400 mb-1">{label}</h4>
        {isCode ? (
            <pre className="bg-slate-900/70 p-3 rounded-lg text-slate-300 text-sm font-mono whitespace-pre-wrap break-words max-h-60 overflow-y-auto">
                <code>{value}</code>
            </pre>
        ) : (
            <p className={`text-slate-200 whitespace-pre-wrap break-words ${isMono ? 'font-mono' : ''}`}>{value}</p>
        )}
    </div>
);
...
const MetadataViewer: React.FC<MetadataViewerProps> = ({ onUsePrompt }) => {
    ...
    if (!imagePreview) {
        return (
            <div className="h-full flex items-center justify-center text-center text-slate-500 bg-slate-900/70 rounded-xl p-8 sticky top-8">
    ...
    return (
        <div className="space-y-6 sticky top-8">
            <div className="bg-slate-900/70 rounded-xl shadow-2xl p-1 backdrop-blur-lg">
                <div className="bg-slate-900 rounded-lg p-6 sm:p-8 space-y-6">
                    <div>
                        <h2 className="text-xl font-semibold text-indigo-400 mb-4">Image Preview</h2>
                        <img src={imagePreview} alt="Uploaded for metadata extraction" className="rounded-xl shadow-lg max-w-full mx-auto" />
                    </div>

                    {extractedMetadata && (
                        <div className="space-y-4 pt-4 border-t border-slate-800">
                            <h3 className="text-xl font-semibold text-indigo-400">Extracted Metadata</h3>
                            ...
                            <div className="space-y-2">
                                <h4 className="font-semibold text-sm text-slate-400">{extractedMetadata.originalPrompt ? "Grounded Prompt" : "Prompt"}</h4>
                                {isEditingPrompt ? (
                                    <textarea
                                        value={displayPrompt}
                                    ...
                                    <pre className={`bg-slate-800/80 p-4 rounded-lg text-slate-300 text-sm overflow-x-auto border max-h-60 whitespace-pre-wrap break-words ${
